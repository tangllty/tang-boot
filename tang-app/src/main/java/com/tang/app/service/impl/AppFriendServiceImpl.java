package com.tang.app.service.impl;

import java.util.List;

import org.springframework.stereotype.Service;

import com.tang.app.entity.AppFriend;
import com.tang.app.mapper.AppFriendMapper;
import com.tang.app.service.AppFriendService;
import com.tang.commons.utils.SecurityUtils;

/**
 * 用户好友业务逻辑层接口实现
 *
 * @author Tang
 */
@Service
public class AppFriendServiceImpl implements AppFriendService {

    private final AppFriendMapper appFriendMapper;

    public AppFriendServiceImpl(AppFriendMapper appFriendMapper) {
        this.appFriendMapper = appFriendMapper;
    }

    /**
     * 查询用户好友列表
     *
     * @param appFriend 用户好友对象
     * @return 用户好友列表
     */
    @Override
    public List<AppFriend> selectAppFriendList(AppFriend appFriend) {
        appFriend.setUserId(SecurityUtils.getUser().getUserId());
        return appFriendMapper.selectAppFriendList(appFriend);
    }

    /**
     * 通过用户好友主键查询用户好友信息
     *
     * @param userFriendId 用户好友主键
     * @return 用户好友信息
     */
    @Override
    public AppFriend selectAppFriendByUserFriendId(Long userFriendId) {
        return appFriendMapper.selectAppFriendByUserFriendId(userFriendId);
    }

    /**
     * 新增用户好友信息
     *
     * @param appFriend 用户好友信息
     * @return 影响行数
     */
    @Override
    public int insertAppFriend(AppFriend appFriend) {
        return appFriendMapper.insertAppFriend(appFriend);
    }

    /**
     * 通过用户好友主键修改用户好友信息
     *
     * @param appFriend 用户好友信息
     * @return 影响行数
     */
    @Override
    public int updateAppFriendByUserFriendId(AppFriend appFriend) {
        return appFriendMapper.updateAppFriendByUserFriendId(appFriend);
    }

    /**
     * 通过用户好友主键删除用户好友信息
     *
     * @param userFriendId 用户好友主键
     * @return 影响行数
     */
    @Override
    public int deleteAppFriendByUserFriendId(Long userFriendId) {
        return appFriendMapper.deleteAppFriendByUserFriendId(userFriendId);
    }

    /**
     * 通过用户好友主键数组批量删除用户好友信息
     *
     * @param userFriendIds 用户好友主键数组
     * @return 影响行数
     */
    @Override
    public int deleteAppFriendByUserFriendIds(Long[] userFriendIds) {
        return appFriendMapper.deleteAppFriendByUserFriendIds(userFriendIds);
    }

}
